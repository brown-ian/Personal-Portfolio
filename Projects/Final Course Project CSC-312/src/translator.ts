...
export function translateExp (e: S.Sexp): L.Exp {
  if (e.tag === 'atom') {
    ...
    if (head.tag === 'atom' && head.value === 'lambda') {
      ...
      if (args.length % 2 !== 0) {
        throw new Error('Parse error: \'rec\' expects an even numver of fields and values')
      } else {
        const newMap = new Map<string, L.Exp>()
        let pos = 0
        while (pos < args.length) {
          const name = args[pos]
          if (name.tag === 'atom') {
            newMap.set(name.value, translateExp(args[pos + 1]))
            pos = pos + 2
          } else {
            throw new Error('Parse error: \'rec\' expects a string as a field name')
          }
        }
        return L.rec(newMap)
      }
      ...
      if (args.length !== 2) {
        throw new Error('Parse error: \'field\' expects a record and a field name')
      } else {
        return L.field(translateExp(args[0]), S.sexpToString(args[1]))
      }
    } else {
