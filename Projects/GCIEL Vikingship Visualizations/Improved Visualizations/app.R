...
dynamicDescription <- function(vis){
  switch(vis,
         ...
         slow all player moved while holding a chosen piece. By graphing this 2D analysis,
         we can determine how long a each person takes to pick up and move a piece, illustrating how
         quickly each piece is picked up, allowing us to determine trends on how long
         people spent viewing the piece, which could be based on intricacy.",
         ...
         visual understanding to how fast or slow one player moved each piece. By graphing this
         2D analysis, we can determine how which players took longer to view each piece
         overall.",
         
         "Player Positions Heatmap" = "This heatmap gives the density of all player 
         positions within the X and Z axis of the VR experience. This graph gives a 
         visual understanding of where players are most likely to be located and 
         where they are least likely to be located.",
         
         "Player Location Animation 1" = "This graph displays the player 
         location over time along the X and Z axis of the VR experience.
         This graph can be used to track player movement over time. This can
         be used to track player movement patterns and can be used to identify
         areas of users' interest.",
         ...
         This can be used to track player movement patterns and can be used to identify
         areas of users' interest.",
         
         "Piece View Amount" = "This graph displays how long each piece in VR was 
      ...
      tabPanel(
        ...
        sidebarLayout(
          sidebarPanel(
            ...
            conditionalPanel(
              # Check the  graph is selected
              condition = "input.tabs == 'use' && input.tabs_use == 'video'",
              ns = NS(NULL),
              
              # Panel title
              h3("Video Engagement Analysis"),
              # Context
              h5(dynamicDescription("Video Engagement Analysis"))
            ),
            ...
            conditionalPanel(
              ...
              # Panel title
              h3("Piece View Amount"),
              # Context
              h5(dynamicDescription("Piece View Amount"))
            ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$updateAnimationButton, {
    ...
    animateLocation <- reactive({
      modData <- inFile() %>% filter(playerID %in% playerIDs)
      
      ggplot(modData, aes(x = Z,
                       y = X,
                       color = playerID)) +
        transition_time(hms::parse_hms(Timestamp)) +
        labs(
          subtitle = "Timestamp: {trunc(frame_time)}",
          x = "Z Coordinate",
          y = "X Coordinate",
  ...
  #15. Piece View Amount Bar Chart Rendering
  output$viewAmountAnimation <- renderImage({
    dataFormatted <- inFile() %>% select(c(playerID, piece, piece_look_time))  %>% 
      mutate(ID = strtoi(str_sub(playerID,start=7))) %>% group_by(piece)
    
    plot <- ggplot(dataFormatted, aes(ID, group = playerID, 
      ...
      geom_text(aes(y=piece_look_time,label = paste(" ", piece_look_time), 
                    hjust=0, size = 8)) +
      ...
      # Remove X and Y data is described in subtitle
      theme(plot.background=element_blank(),
            ...
            # Lines indicating ratio if wanted
    ...
    animatedPlot <- plot + transition_states(piece, transition_length = 10, state_length = 2) +
      labs(title = 'Viking Ship Piece : {closest_state}',  
           subtitle  =  "Piece View Amount (Seconds)")
