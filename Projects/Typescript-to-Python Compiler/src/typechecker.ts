...
export function typecheck (ctx: L.Ctx, e: L.Exp): L.Typ {
  switch (e.tag) {
    ...
    case 'rec': {
      const typeMap = new Map<string, L.Typ>()

      for (let key of e.map.keys()) {
        let val = e.map.get(key)
        if (val === undefined) {
          throw new Error(`Type error: there is no value field for ${key} inside of the record`)
        } else {
          let typechecked = typecheck(ctx, val)
          typeMap.set(key, typechecked)
        }
      }
      return L.tyrec(typeMap)
    }

    case 'field': {
      const v = typecheck(ctx, e.e)

      if (v.tag === 'rec') {
        let vtype = v.map.get(e.f)
        if (vtype === undefined) {
          throw new Error(`Type error: the field ${e.f} is not inside of the record: Typechecker`)
        } else {
          return vtype
        }
      } else {
        throw new Error(`Type error: 'if' expects a rec in primary position but a ${v.tag} was given.: typechecker`)
      }
    }
  }
}
